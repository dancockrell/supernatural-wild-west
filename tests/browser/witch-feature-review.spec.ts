...
test('witch uses dedicated native performance without upscaling or early dismissal',async({page})=>{
 ...
 await page.screenshot({path:'docs/witch-feature-runtime-review.png'});
 expect(await clip.evaluate((v:HTMLVideoElement)=>v.duration)).toBeGreaterThanOrEqual(8);
 await expect.poll(()=>clip.evaluate((v:HTMLVideoElement)=>v.currentTime),{timeout:10000}).toBeGreaterThan(6.1);
