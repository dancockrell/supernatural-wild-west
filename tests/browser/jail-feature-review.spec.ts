...
test('jail uses dedicated native performance without upscaling or early dismissal',async({page})=>{
 await page.setViewportSize({width:3840,height:2160});
 await page.goto('/?parlor=1'); await preview(page,'awaken-2');
 const clip=page.locator('.feature-stage video');
 await expect(clip).toHaveAttribute('src','/video/feature-performances-v2/jail.webm');
 ...
 await page.screenshot({path:'docs/jail-feature-runtime-review.png'});
