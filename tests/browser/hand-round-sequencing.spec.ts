...
import {clientBundle,clientFunction,transpile} from './client-module';
const fn=(name:string)=>clientFunction('main',name);
test('a settled round awaits its hand film and then the feature chain before completing',async({page})=>{
  await page.setContent('<main></main>');
  await page.addScriptTag({content:transpile(`
...
test('initial-state refresh preserves an active preview and settled-state notification restores controls',async({page})=>{
 ...
 await page.addScriptTag({content:clientBundle({
  modules:['poker-table','ghost-hand','poker-guests'],
  stubs:{
   cardFace:"()=>''",handChoreographyIndices:'()=>[]',handMotion:"()=>({name:'none'})",stageHandFrames:'frames=>frames',
   ghostSprite:"()=>document.createElement('video')",parlorResidentMedia:'()=>({})',
  },
  append:`
 const state={balance:1000,poker:{cards:[]},phase:'noon',awakened:[],sequence:0,configVersion:'test'},lastResult=undefined;
 const elements=new Map();const el=id=>{if(!elements.has(id))elements.set(id,{hidden:true,parentElement:{},style:{},dataset:{},classList:{toggle(){},add(){},remove(){}}});return elements.get(id)};
