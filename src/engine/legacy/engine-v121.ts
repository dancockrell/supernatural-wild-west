import { resolveSpin as resolvePoker120 } from "./engine-v120";
import { resolveSpin as resolveLegacy } from "./engine-v1";
import { collectCard } from "./poker-v12";
import { CONFIG } from "./config-v121";
import {
  REGULAR,
  LOCATIONS,
  type GameState,
...
export function resolveSpin(
  ...
  // Cosmetic face identities are derived without consuming RNG or changing payout probabilities.
  // The selected collection cell always displays the actual card added to the hand.
  const cardFaces: Record<number, number> = {};
  const anchor = poker ? poker.cards[poker.cards.length - 1] : stops[0][0] % 52;
  for (let cell = 0; cell < 20; cell++) {
    if (grid[Math.floor(cell / 4)][cell % 4] === "dust")
      cardFaces[cell] = (anchor + cell - (poker?.cell || 0) + 52) % 52;
  }
  return {
    schemaVersion: 1,
    configVersion: CONFIG.version,
