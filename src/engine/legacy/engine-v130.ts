import { resolveSpin as resolvePoker121 } from "./engine-v121";
import { resolveSpin as resolvePoker120 } from "./engine-v120";
import { resolveSpin as resolveLegacy } from "./engine-v1";
import { settleBoard } from "./poker-v130";
import { CONFIG } from "./config-v130";
import {
  REGULAR,
  LOCATIONS,
  type GameState,
...
export function resolveSpin(
  ...
  const { poker, cardFaces } = settleBoard(grid, bet, rng);
  const wins = poker?.outcome === "win" ? matchedWins : [];
  if (poker) {
    events.push({
      type: "poker-deal",
      cells: poker.cells,
