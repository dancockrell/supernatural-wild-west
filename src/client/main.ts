...
function showAnimationPreview() {
  ...
  for (const [key,label] of [['queen','Lantern maiden'],['medium','Brazier maiden']] as const) {
    ...
    const labels=key==='queen'
      ? ['quiet watch','reaches out','shields her lantern','adjusts her coat','shivers','reacts to a win']
      : ['quiet reflection','lifts her bowl','listens','looks toward the room','inclines her head','reacts to a win'];
    [media.idle,media.alternate,media.characterIdle,...media.quietVariants,media.reaction]
      .forEach((src,i)=>idles.push([`${label} - ${labels[i]}`,src]));
  }
  ...
  el('modal-body').insertAdjacentHTML('beforeend', `<h3>Characters</h3><p>Each maiden has five idle performances and a win reaction. Their larger performances are included in the feature events and poker hands above.</p><div class="feature-gallery">${idles.map(([label],i)=>`<button class="outline-button" data-character-preview="${reactions.length+i}">${label}</button>`).join('')}</div>`);
