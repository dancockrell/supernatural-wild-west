...
function showAnimationPreview() {
  ...
  for (const [key,label] of [['queen','Lantern maiden'],['medium','Brazier maiden']] as const) {
    ...
    idles.push([`${label} - ${key === 'queen' ? 'quiet watch' : 'quiet reflection'}`,media.idle]);
  }
  const lantern = parlorResidentMedia('queen');
  idles.push(['Lantern maiden - reaches out', lantern.alternate],
    ['Lantern maiden - shields her lantern', lantern.characterIdle],
    ['Lantern maiden - reacts to a win', lantern.reaction]);
  const brazier = parlorResidentMedia('medium');
  idles.push(['Brazier maiden - looks toward the room', brazier.alternate],
    ['Brazier maiden - listens to the bowl', brazier.characterIdle],
    ['Brazier maiden - reacts to a win', brazier.reaction]);
  ...
  el('modal-body').insertAdjacentHTML('beforeend', `<h3>Characters</h3><p>Each maiden has four recorded performances. Their quiet actions cycle in the room, and good results bring a reaction. The brazier maiden's existing performances are restored; visual cleanup remains in progress.</p><div class="feature-gallery">${idles.map(([label],i)=>`<button class="outline-button" data-character-preview="${reactions.length+i}">${label}</button>`).join('')}</div>`);
