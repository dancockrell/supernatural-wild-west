...
await page.locator('#spin').click();await page.waitForFunction(()=>JSON.parse(localStorage.getItem('sww-public-demo-v1')||'null')?.state.sequence>0);await page.waitForFunction(()=>!document.querySelector('#spin').disabled && document.querySelector('#spin').textContent.includes('SPIN'),null,{timeout:45000}).catch(async e=>{console.log(await page.locator('#spin').textContent(),errors,await page.locator('body').innerText());throw e;});
